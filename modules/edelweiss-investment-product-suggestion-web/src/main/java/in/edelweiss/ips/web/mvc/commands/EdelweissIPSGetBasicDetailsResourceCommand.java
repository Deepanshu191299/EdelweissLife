...
@Component(
	    ...
	    property = {
	        ...
	        "mvc.command.name=/get/ipsBasicDetails"
	    },
	    service = MVCResourceCommand.class
	)
public class EdelweissIPSGetBasicDetailsResourceCommand extends BaseMVCResourceCommand {

	private static final Log log = LogFactoryUtil.getLog(EdelweissIPSGetBasicDetailsResourceCommand.class);
	...
	protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)throws PortletException {
		...
		if(Validator.isNotNull(leadId)) {
			try {
				...
				log.debug("EdelweissIPSGetBasicDetailsResourceCommand >>> serveResource >>>> url to get basic details >>>>> "+customerEnquiryAPIURL);
				...
				log.error("EdelweissIPSGetBasicDetailsResourceCommand >>> serveResource >>> An error occurred getting the storing Basic details ::: "+e);
			} catch (Exception e) {
				log.error("EdelweissIPSGetBasicDetailsResourceCommand >>> serveResource >>> IOException ::: "+e);
			}
		}
	}
