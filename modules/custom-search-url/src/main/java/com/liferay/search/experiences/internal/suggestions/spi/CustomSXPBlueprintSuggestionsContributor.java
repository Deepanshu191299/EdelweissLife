...
@Component(
	...
	enabled = true,
	property = {
		"search.suggestions.contributor.name=sxpBlueprint",
		"component.name=com.liferay.search.experiences.internal.suggestions.spi.SXPBlueprintSuggestionsContributor",
		"service.ranking:Integer=22434"
	},
	service = SuggestionsContributor.class
)
public class CustomSXPBlueprintSuggestionsContributor
	extends SXPBlueprintSuggestionsContributor {
	...
	private String _getAssetURL(
		...
		try {
			...
			viewURL = viewURL.replaceAll("/web/guest/b/", "/blogs/");
			viewURL = viewURL.replaceAll("/web/guest/blogs/", "/blogs/");
	...
	private Suggestion _getSuggestion(
		...
		if (useAssetTitle) {		
			...
			if(entryClassName.equalsIgnoreCase(BlogsEntry.class.getName())) {
				try {
					String blogTitle = (String)_blogEntryLocalService.getBlogsEntry(entryClassPK).getExpandoBridge().getAttribute("BlogTitle");
					suggestionBuilder.text(blogTitle);
				} catch (PortalException e) {
					suggestionBuilder.text(assetRenderer.getTitle(searchContext.getLocale()));
				}
			} else {
				suggestionBuilder.text(assetRenderer.getTitle(searchContext.getLocale()));
			}
	...
	private static final Log _log = LogFactoryUtil.getLog(
		CustomSXPBlueprintSuggestionsContributor.class);
	...
	@Reference
	private BlogsEntryLocalService _blogEntryLocalService;
	@Reference
