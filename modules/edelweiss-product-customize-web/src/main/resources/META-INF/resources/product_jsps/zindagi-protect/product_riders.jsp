...
<c:if test="<%=Validator.isNotNull(productRidersList) && productRidersList.length()>0%>">
	<div id="<portlet:namespace/>custome-plan" class="">
		...
		<div id="<portlet:namespace/>productRidersContainer" class="custome-plan row">
			...
			<div class="custom-card-wrapper benefit-card row">
				...
				for(int i=0; i<productRidersList.length();i++){
					...
					String isVisible = StringPool.BLANK;
					
					JSONObject productRider=productRidersList.getJSONObject(i);
					
					if(Validator.isNull(productRider)){
						continue;
					}
					
					String riderId=productRider.getString("riderId").replace(StringPool.SPACE, StringPool.BLANK);
					if(!productRider.getBoolean("isActive")) { continue; }

					String key = Validator.isNotNull(productRider.getString("key")) ? productRider.getString("key") : StringPool.BLANK;

					if(productRider.getBoolean("isBenefit")){
						if(key.equals("BHB")){
							/* Validations:To show BHB Card */
							if(!Validator.isNotNull(familyDetailsResponseJson)) {
								isVisible = "d-none";
							} else if(!Validator.isNotNull(familyDetailsResponseJson.getString("spouseName"))){
								isVisible = "d-none";
							};	
						 }else if(key.equals("CHILD")){ 
							/* Validations:To show CFPB Card */
							 if(!Validator.isNotNull(familyDetailsResponseJson)) {
								 isVisible = "d-none";
							 } else if(!Validator.isNotNull(familyDetailsResponseJson.getString("childDateOfBirth"))){
								 isVisible = "d-none";
							 };
						}
				%>
				<div id="<portlet:namespace/>rider_<%=key %>" class="col-12 col-md-6 zp-benefits productRider p-0 m-0 <%=isVisible %>">
					<div class="m-2 custom-card">
							...
							<h2><%=productRider.getString("title", StringPool.BLANK) %></h2>
							  <div class="edto-check-content p-0 mx-0 mt-1" style=" width: auto; ">
							  	<div class="form-group">
									<input name="<portlet:namespace/>selectedRiders" type="checkbox" value="<%=key %>" id="<portlet:namespace/><%="selectedRider_"+key %>" 
									...
									<label class="form-check-label" for="<portlet:namespace/><%="selectedRider_"+key %>"></label>
								</div>
							</div>
						</div>
						<div id="<portlet:namespace/>riderBrochure_<%=key %>" class="">
						...
						<div id="<portlet:namespace/>riderPremiumDetails_<%=riderId %>" class="row mt-2 price-tile">
							<div id="<portlet:namespace/>riderPriceDetails_<%=riderId %>" class="col-6 custom-price">
								<p>	<span>@</span><span id="<portlet:namespace/>riderPriceAmount_<%=key %>"></span></p>
							</div>
							<%if(key.equals("CHILD")){ %>
								<div id="<portlet:namespace/>childBenefitSAPercentage<%=riderId %>" class="col-6">
									<aui:select name="<%="childFutureProtectBenefit" %>" id="<%="childFutureProtectBenefit" %>" label="" cssClass="riderPremiumInput onlyBottomBorder" data-rider-id="<%=riderId %>">
											<c:forEach var="curOption" items="${childBenefitSAPercentagePicklistData}">
												<aui:option value="${curOption.key}" >
													${curOption.value}
												</aui:option>
											</c:forEach>
									</aui:select>
						...
						<div class="family-income-benefit-error overlay-card card-body my-family-body" id="<portlet:namespace/>BHBEntryAgeError_<%=riderId %>" style="display:none">
                            <p class="overlay-card-text"><liferay-ui:message key="label-not-applicable-with-BHB" arguments="<%= productRider.getString("title", StringPool.BLANK)  %>"/></p>
                        </div>
				...
				<div id="<portlet:namespace/>rider_<%=riderId %>" class="col-12 col-md-6 zp-riders productRider p-0 m-0">
					<div class="m-2 custom-card">
						<div class="d-flex justify-content-between" style="column-gap: 10px;align-items: flex-start;">
							<h2><%=productRider.getString("title", StringPool.BLANK) %></h2>
							  <div class="edto-check-content p-0 mx-0 mt-1" style=" width: auto; ">
...
<script>
	...
	$(document).ready(function() {
	    var divs = $(".zp-benefits");
	    divs.each(function(index, element) {
	        $(".benefit-card").prepend(element);
	    });

	    var divs = $("#<portlet:namespace/>rider_CHILD");
	    divs.each(function(index, element) {
	        $(".benefit-card").prepend(element);
	    });
	 });
