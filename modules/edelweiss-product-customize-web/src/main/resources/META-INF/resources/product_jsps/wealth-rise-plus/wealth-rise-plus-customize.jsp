...
<div class="wealth-small-text">
        <p class="para3 text-center">
...
<c:choose>
	<c:when test="${isProductSelected}">
		<form name="<portlet:namespace />customizeForm"  id="<portlet:namespace />customizeForm" method="post" action="${saveInvestmentDetails}">
			...
			<input type="hidden" name="<portlet:namespace />fundValuetobeWithdrawn" id="<portlet:namespace />fundValuetobeWithdrawn"  value="${productCustomizeSummaryData['productDefaultValueData'].fundValueToBeWithdrawn.key}"/>
			<input type="hidden" name="<portlet:namespace />policyYearFromWhichSWPPayable" id="<portlet:namespace />policyYearFromWhichSWPPayable"  value="${productCustomizeSummaryData['productDefaultValueData'].policyYearFromWhichSWPPayable.key}"/>
...
$(document).ready(function () {
	...
	var config = new Object({}),
		 ...
		 config.policyTermPolicyForRelData = '${policyTermPolicyForRelData}';
		 ...
		 config.paymentOptionRelations = JSON.parse('${productCustomizeSummaryData["paymentOptionRelations"]}');
		 config.productPolicyForRel = JSON.parse('${productCustomizeSummaryData["productPolicyForRel"]}');
...
<script src="<%=request.getContextPath()%>/js/wealth-rise-plus.js?t=<%=EdelweissProductCustomizeConstants.CURRENT_TIME_STAMP%>"></script>
