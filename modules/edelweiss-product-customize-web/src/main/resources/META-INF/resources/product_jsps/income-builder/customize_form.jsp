...
<%@ include file="/product_jsps/income-builder/edit_basic_details.jsp" %>
...
<c:choose>
	<c:when test="${isProductSelected}">
					...
					<div class="edelweissFormInner d-flex">
						<div id="edelweissTokioForm1" class="edelweissTokioForm">
							<div class="row owl-carousel owl-theme">
										...
										<div class="labelInputGroup">
											...
											<div class="select-container ">
												<select class="form-control" id="<portlet:namespace />policyTerm" name="<portlet:namespace />policyTerm">
													<c:forEach var="curPolictTerm" items="${ibPolicyTermPicklistData}">
										...
										<div class="labelInputGroup">
											...
											<div class="select-container ">
												<select class="form-control" id="<portlet:namespace />policyFor" name="<portlet:namespace />policyFor">
													<c:forEach var="curPayFor" items="${ibPayForPicklistData}">
								...
								<c:if test="${productCustomizeSummaryData['productDefaultValueData'].isIBPayoutOptionApplicable()}">
									<div class=" item" id="<portlet:namespace />payoutOptionDiv">
										<div class="labelInputGroup">
											<div class="labelGroup">
												<label for="<portlet:namespace />payoutOption" class="pform-title mb-0"><liferay-ui:message key="payoutOption" /></label>
												<p class="premiuim-small-text">
													<liferay-ui:message key="payout-option" />
												</p>
											</div>
											<div class="select-container ">
												<select class="form-control" id="<portlet:namespace />payoutOption" name="<portlet:namespace />payoutOption">
													<c:forEach var="curPayoutOption" items="${ibPayoutOptionPicklistData}">
														<option data-name="${curPayoutOption.value}" value="${curPayoutOption.key}"
															${productCustomizeSummaryData['productDefaultValueData'].iBPayoutOption.key == curPayoutOption.key ? 'selected' : ''}>${curPayoutOption.value}</option>
													</c:forEach>
												</select>
											</div>
										</div>
								...
								<c:if test="${productCustomizeSummaryData['productDefaultValueData'].isIBPayoutPeriodApplicable()}">
									<div class=" item" id="<portlet:namespace />payoutPeriodDiv">
										<div class="labelInputGroup">
											<div class="labelGroup">
												<label for="<portlet:namespace />payoutPeriod" class="pform-title mb-0"><liferay-ui:message key="payoutPeriod" /></label>
												<p class="premiuim-small-text">
													<liferay-ui:message key="payout-period" />
												</p>
											</div>
											<div class="select-container ">
												<select class="form-control" id="<portlet:namespace />payoutPeriod" name="<portlet:namespace />payoutPeriod">
													<c:forEach var="curPayoutPeriod" items="${ibPayoutPeriodPicklistData}">
														<option data-name="${curPayoutPeriod.value}" value="${curPayoutPeriod.key}"
															${productCustomizeSummaryData['productDefaultValueData'].iBPayoutPeriod.key == curPayoutPeriod.key ? 'selected' : ''}>${curPayoutPeriod.value}</option>
													</c:forEach>
												</select>
											</div>
										</div>
		...
		<%@ include file="/product_jsps/income-builder/family-details.jsp"%>
		<%@ include file="/product_jsps/income-builder/product_features.jsp"%>
		<%@ include file="/product_jsps/income-builder/product_riders.jsp"%>
	</c:when>
	<c:otherwise>
...
LANG_MESSAGES["age-between-6-to-17-years"] = "<liferay-ui:message key='age-between-6-to-17-years' />";
...
try{
	selectedRidersIds = (parseJSONString('${productCustomizeSummaryData["productDefaultValueData"].selectedRidersIds}'));
}catch(e){
	console.error(e);
}
$(document).ready(function () {
	...
	var config = new Object({}),
		 ...
		 payoutPeriodEl=namespace + "payoutPeriod",
		 payoutPeriodJEl="#" + payoutPeriodEl,
		 ...
		 config.payoutOptionEl=payoutOptionEl;
		 config.payoutOptionJEl=payoutOptionJEl;
		 config.payoutPeriodEl=payoutPeriodEl;
		 config.payoutPeriodJEl=payoutPeriodJEl;
		 ...
		 config.ibRelFieldValues = JSON.parse('${ibRelatedFieldsData}');
...
<script src="<%=request.getContextPath()%>/product_jsps/income-builder/js/ib-main.js?t=<%=EdelweissProductCustomizeConstants.CURRENT_TIME_STAMP%>""></script>
<%-- <script src="<%=request.getContextPath()%>/js/basic_detail.js?t=${currentTimeStamp}"></script> --%>
<script src="<%=request.getContextPath()%>/product_jsps/income-builder/js/ib-basic-details.js?t=<%=EdelweissProductCustomizeConstants.CURRENT_TIME_STAMP%>""></script>
...
<script type="text/javascript">
	...
	$("#<portlet:namespace/>proceedBtn").click(function() {
		...
		var eventname = "income Builder 3";
		...
		guaranteedBenefit = document.getElementById('<portlet:namespace />totalGuaranteedBenefitAmount').innerText.replaceAll(',','');
		planOption = $('#<portlet:namespace />policyOption').find(":selected").text();
		premiumPaymentTerm = $('#<portlet:namespace />policyFor').find(":selected").val();
		...
		benefitDuration = $('#<portlet:namespace />payoutPeriod').find(":selected").text();
		...
		benefitType = $('#<portlet:namespace />payoutOption').find(":selected").text();
