...
<%
	...
	JSONArray productRidersList = liferayProductNestedData.getJSONArray("items");
				...
				%>
					...
					<div class="m-2 custom-card">
						<div class="d-flex justify-content-between" style="column-gap: 10px;align-items: flex-start;">
							<h2><%=productRider.getJSONObject("productRider").getString("title", StringPool.BLANK) %></h2>
							  <div class="edto-check-content p-0 mx-0 mt-1" style=" width: auto; ">
							  	<div class="form-group">
									<input name="<portlet:namespace/>selectedRiders" type="checkbox" value="<%=riderId %>" id="<portlet:namespace/><%="selectedRider_"+riderId %>" 
										data-rider-price="" data-rider-name="<%=productRider.getJSONObject("productRider").getString("title", StringPool.BLANK) %>" 
										data-rider-key="<%=productRider.getJSONObject("productRider").getString("key", StringPool.BLANK) %>" class="riderSelectionInput"/>
						...
						<div id="<portlet:namespace/>riderBrochure_<%=riderId %>" class="">
							<c:if test="<%=Validator.isNotNull(productRider.getJSONObject("productRider").getJSONObject("brochure")) && 
								Validator.isNotNull(productRider.getJSONObject("productRider").getJSONObject("brochure").getJSONObject("link"))%>" >
								<a href="<%=productRider.getJSONObject("productRider").getJSONObject("brochure").getJSONObject("link").getString("href","#") %>" 
									download="<%=productRider.getJSONObject("productRider").getJSONObject("brochure").getJSONObject("link").getString("href",StringPool.BLANK)  %>">
										<liferay-ui:message key="label-download-brochure"/>
								</a>
							</c:if>
						</div>
						<div id="<portlet:namespace/>riderDescription<%=riderId %>">
							<%=productRider.getJSONObject("productRider").getString("description", StringPool.BLANK) %>
						</div>
						<div id="<portlet:namespace/>riderPremiumDetails_<%=riderId %>" class="row mt-2 price-tile">
							...
							<c:if test="<%=productRider.getJSONObject("productRider").getString("key", StringPool.BLANK).equalsIgnoreCase("PayorWaiverBenefitRider") %>" >
						...
						<div class="family-income-benefit-error overlay-card card-body my-family-body" id="<portlet:namespace/>familyIncomeBenefitError_<%=riderId %>" style="display:none">
 							<p class="overlay-card-text"><liferay-ui:message key="label-not-applicable-with-family-income-benefit-opt" arguments="<%= productRider.getJSONObject("productRider").getString("title", StringPool.BLANK)  %>"/></p>
						</div>
						<div class="family-income-benefit-error overlay-card card-body my-family-body" id="<portlet:namespace/>WOPEntryAgeError_<%=riderId %>" style="display:none">
 							<p class="overlay-card-text"><liferay-ui:message key="label-not-applicable-with-WOP" arguments="<%= productRider.getJSONObject("productRider").getString("title", StringPool.BLANK)  %>"/></p>
						</div>
						<div class="family-income-benefit-error overlay-card card-body my-family-body" id="<portlet:namespace/>WOPAgeError_<%=riderId %>" style="display:none">
 							<p class="overlay-card-text"><liferay-ui:message key="label-not-applicable-with-WOP-age" arguments="<%= productRider.getJSONObject("productRider").getString("title", StringPool.BLANK)  %>"/></p>
						</div>
						<div class="available-for-myself overlay-card card-body my-family-body" id="<portlet:namespace/>forMySelf_<%=riderId %>" style="display:none">
							<p class="overlay-card-text">	<liferay-ui:message key="label-only-for-myself-investmentes" arguments="<%= productRider.getJSONObject("productRider").getString("title", StringPool.BLANK)  %>"/>	</p>					
						</div>
						<div class="available-for-family overlay-card card-body my-family-body" id="<portlet:namespace/>forFamily_<%=riderId %>" style="display:none">
							<p class="overlay-card-text">	<liferay-ui:message key="label-only-for-family-investmentes" arguments="<%= productRider.getJSONObject("productRider").getString("title", StringPool.BLANK)  %>"/>	</p>				
						</div>
						<div class="available-for-family overlay-card card-body my-family-body" id="<portlet:namespace/>criticalIllnessRiderError_<%=riderId %>" style="display:none">
                            <p class="overlay-card-text">        <liferay-ui:message key="label-for-critical-illness-rider-condition" arguments="<%= productRider.getJSONObject("productRider").getString("title", StringPool.BLANK)  %>"/>        </p>                                
                        </div>                     
