...
<portlet:resourceURL id="/combo/generate-bi" var="edelweissGenerateBIURL">
</portlet:resourceURL>
...
<c:choose>
	<c:when test="${isProductSelected}">
		<form name="<portlet:namespace />customizeForm" id="<portlet:namespace />customizeForm" method="post" action="${saveInvestmentDetails}">
			...
			<input type="hidden" name="<portlet:namespace />premiumAmount" id="<portlet:namespace />premiumAmount" />
									...
									<div class="item">
										<div class="labelInputGroup">
											...
											<div class="pos">
												...
												<input type="text" class="form-control border-0" id="<portlet:namespace />investmentAmount" name="<portlet:namespace />investmentAmount" value="${productCustomizeSummaryData['productDefaultValueData'].getInvestmentAmount()}">
											</div>
										...
										<div class="labelInputGroup">
											...
											<div class="select-container ">
												<select class="form-control" id="<portlet:namespace />policyTerm" name="<portlet:namespace />policyTerm">
													<option value=""><liferay-ui:message key="select" /></option>
													<%-- <c:forEach var="curWSPPolicyTerm" items="${wspPolicyTermPicklistData}">
														<option data-name="${curWSPPolicyTerm.value}" value="${curWSPPolicyTerm.value}" ${productCustomizeSummaryData['productDefaultValueData'].swp.key == curWSPPolicyTerm.key ? 'selected' : ''}>${curWSPPolicyTerm.value}</option>
													</c:forEach> --%>
												</select>
											</div>
										</div>
										...
										<div class="labelInputGroup">
											...
											<div class="select-container ">
												<select class="form-control" id="<portlet:namespace />payFor" name="<portlet:namespace />payFor">
													<option value=""><liferay-ui:message key="select" /></option>
													<%-- <c:forEach var="curWSPPayFor" items="${wspPayForPicklistData}">
														<option data-name="${curWSPPayFor.value}" value="${curWSPPayFor.value}" ${productCustomizeSummaryData['productDefaultValueData'].swp.key == curWSPPayFor.key ? 'selected' : ''}>${curWSPPayFor.value}</option>
													</c:forEach> --%>
												</select>
											</div>
										</div>
						...
						<div class="edelweissTokioTotalReturns">
							...
							<div class="left">
								...
								<div class="histy-amt-wrpr d-flex align-items-end">
									<div class="histy-amt-content d-flex justify-content-between w-100">
		                             ...
		                             <div class="break-proceed-group">
		                             	<input type="button" id="<portlet:namespace />breakUpBtn" value="Break-up" 
		                             		class="btn btn-primary breakup_btn float-left csp-breakUpBtn disabled-btn d-none d-md-block d-lg-block">	
										<input type="button" id="<portlet:namespace />proceedBtn" value="PROCEED" class="btn edto-btn-primary float-right">
									</div>
								</div>
							</div>
						</div>
		...
		<div class="edelweissTokioDetails p-2 d-block d-md-none d-lg-none">
			<div class="container">
				<div class="edelweissTokioDetailsInner d-flex align-items-center" style=" justify-content: space-between;">
					<div class="edelweissTokioHeading">
						<h5 class="fontbold d-block d-md-none d-lg-none basic-details">Plan Details</h5>
					</div>
					<input type="button" id="<portlet:namespace />breakUpBtn" value="Break-up" class="btn btn-primary breakup_btn float-left csp-breakUpBtn csp-breakUpBtnMob ">
				</div>
			</div>
		</div>
		<div>
			<%@ include file="break-up.jsp" %>
		</div>
...
$(document).ready(function () {
	var config = new Object({}),
		 ...
		 config.wspPolicyTermPicklistData = '${capitalSecurePolicyTermDataMap}';
		 config.wspPayForPicklistData = '${capitalSecurePayForDataMap}';
		 ...
		 config.isFormSubmitPendingJEl = isFormSubmitPendingJEl;
...
<script src="<%=request.getContextPath()%>/js/capital-secure-plus.js?t=${currentTimeStamp}"></script>
...
<script type="text/javascript">
	...
	$("#<portlet:namespace />proceedBtn").click(function() {
		...
		spouseName = $('#<portlet:namespace/>spouseFullName').val()===undefined?'':$('#<portlet:namespace/>spouseFullName').val();
		spouseDOB = $('#<portlet:namespace/>spouseDOB').val()===undefined?'':$('#<portlet:namespace/>spouseDOB').val();
