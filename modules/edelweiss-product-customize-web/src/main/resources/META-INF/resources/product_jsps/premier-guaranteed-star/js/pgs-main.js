		  ...
		  */}
			...
			/*updateGetIncomeForValue =function(isOnLoad){
			    ...
			    var payForValue=document.getElementById(policyForEl).value
						pgiRelFieldValues.forEach(field=>{
			                if(field.payFor.key==payForValue){
			...
			updatePolicyTermValue =function(isOnLoad){
					...
					var age = calculateAge(dateOfBirthInputVal);
							pgiRelFieldValues.forEach(field=>{
				                if((field.payFor.key==payForValue) && ((parseInt(age)+parseInt(field.policyTerm.key))<100)){
...
*/				
			...
			function renderMaturityBenefitsBreakup(biResponseData){
				...
				productBIJson.forEach(biJson=>{
					...
					incomeBenefitPayoutData.appendChild(document.createTextNode(biJson.INCOME_BEN_PAY_OUT!=0?rupeeSignLabel.concat(Math.round(biJson.INCOME_BEN_PAY_OUT).toLocaleString("en-IN")):'-'))
					tableRecord.appendChild(incomeBenefitPayoutData);
						
					maturityBenefitData.appendChild(document.createTextNode(biJson.MATURITY_BEN!=0?rupeeSignLabel.concat(Math.round(biJson.MATURITY_BEN).toLocaleString("en-IN")):'-'))
					tableRecord.appendChild(maturityBenefitData);
					
					deathBenefitData.appendChild(document.createTextNode(biJson.DEATH_BEN!=0?rupeeSignLabel.concat(Math.round(biJson.DEATH_BEN).toLocaleString("en-IN")):'-'))
			...
			function renderIncomeBenefitsBreakup(biResponseData){
				...
				var benefitAmount = 0;
				var incomeBenefitAmount = 0;
				var maturityBenefitAmount = 0;
				...
				productBIJson.forEach(biJson=>{
					...
					if(biJson.INCOME_BEN_PAY_OUT != 0 || biJson.MATURITY_BEN !=0){	
						...
						incomeBenefitAmount = biJson?.INCOME_BEN_PAY_OUT != 0 ? Math.round(biJson.INCOME_BEN_PAY_OUT) : 0;
						maturityBenefitAmount = biJson?.MATURITY_BEN != 0 ? Math.round(biJson.MATURITY_BEN) : 0;
						
						benefitAmount = benefitAmount + incomeBenefitAmount + maturityBenefitAmount ;
					}
				});
			...
			getBIRequestProduct = function(requestedRidersList, loadDefaultValues) {
				...
				console.log("lumpsumBenifit: "+lumpsumBenifit);
				var product = {
					"category": "1",
...
/**--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------**/			
			...
			loadCustomersSavedRiders = function(){
				console.log("Inside loadCustomersSavedRiders");
				if(!selectedRidersIds || !selectedRidersIds.length){
					console.log("Inside loadCustomersSavedRiders if");
					return;
				}
				selectedRidersIds.forEach(function(rider){
					console.log("Calling updateRiderData");
			...
			updateRiderData = function(rider){
				console.log("Inside updateRiderData"+rider.id);
				var riderElement = document.getElementById(namespace+"selectedRider_"+rider.id);
				console.log("riderElement"+riderElement);
			...
			renderRiderPremiumAmount = function(biResponseJson){
				...
				biResponseJson.InputValidationStatus.forEach(function(productRider){
			    	...
			    	{
				    	document.getElementById(namespace+"selectedRider_"+riderId).dataset.riderPrice = Math.round(riderPremiumAmount);
			...
			addRemoveRider = function(selectedRiderId, isAdded){
				console.log("selectedRiderId: "+selectedRiderId+",isAdded: "+isAdded);
				...
				if(isAdded && selectedRider.dataset.isAvailable=="true"){
					console.log("isAdded: "+isAdded+",isAvailable: "+selectedRider.dataset.isAvailable);
					...
					if(document.getElementById(namespace+"riderPremiumAmount_"+selectedRider.value)){
						console.log("rider value: "+document.getElementById(namespace+"riderPremiumAmount_"+selectedRider.value));
					...
					if(document.getElementById(namespace+"riderPremiumAmount_"+selectedRider.value)){
						console.log("rider premium amount: "+document.getElementById(namespace+"riderPremiumAmount_"+selectedRider.value));
			...
			validateProductRiders = function(){
				var dateOfBirthInputVal = $(dateOfBirthJEl).val();
				var lumpSumBenefitVal = $(lumpSumBenefitJEl).val();
				var age = calculateAge(dateOfBirthInputVal);
				console.log("age......"+age);
				console.log("lumpSumBenefitVal......"+lumpSumBenefitVal);
				/*if((age>50 && lumpSumBenefitVal== 2) || (age>55 && lumpSumBenefitVal== 1)){
					$(policyForJEl+" option[value='5']").remove();
					if($(policyForJEl).val()==8){
						$(policyTermJEl).empty();
						$(policyTermJEl).append("<option value='20'>20</option>");
						$(policyTermJEl).append("<option value='25'>25</option>");
						$(policyTermJEl).append("<option value='30'>30</option>");
						$(policyTermJEl).append("<option value='35'>35</option>");
						$(policyTermJEl).append("<option value='40'>40</option>");
					}
				}else if($(policyForJEl+" option[value='5']").val()!=5){
					$(policyForJEl).prepend("<option value='5'>5</option>");
				}*/
				
				if($(policyForJEl+" option[value='5']").val()!=5){
					$(policyForJEl).prepend("<option value='5'>5</option>");
				}
				...
				riderValidations.forEach((riderValidation,riderId)=>{
					...
					console.log("isInvestingForValid: "+isInvestingForValid);
					console.log("isFamilyIncomeBenefitValid: "+isFamilyIncomeBenefitValid);
					console.log("isEntryAgeValid: "+isEntryAgeValid);
			...
			validateEntryAge = function(riderId){
			    ...
			    }else if(!isRiderAvailable && PPT+age>70){
			...
			validateFamilyIncomeBenefit = function(riderValidation,riderId){
			    ...
			    console.log("isRiderAvailable: "+isRiderAvailable);
			...
			generateSelectedRidersRequestBody = function(requestedRidersList, loadDefaultValues){
				...
				document.getElementsByName(namespace+"selectedRiders").forEach(availableRider=>{
                    ...
                    if(document.getElementsByName(namespace+"selectedRiders").value == "50003" && age > 60){
                             riderDetailsRequestBody[availableRider.dataset.riderKey]="";
                             return;
				    }
...
/**--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------**/
			...
			init = async function(){
				var oldURL = sessionStorage.previousURL;
				console.log("oldURL..."+oldURL);
				
				if((typeof oldURL !== "undefined") && oldURL.includes("/buy")){
					sessionStorage.removeItem("previousURL");
					sessionStorage.removeItem("selectVal");
					sessionStorage.removeItem("lumpSumBenefitVal");
				}else{
					if (sessionStorage.selectVal) {
						$(incomePayoutFrequencyJEl).val( sessionStorage.selectVal );
					}if(sessionStorage.lumpSumBenefitVal){
						$(lumpSumBenefitJEl).val(sessionStorage.lumpSumBenefitVal);
					}
				}
					$(incomePayoutFrequencyJEl).on('change', function(){
					...
					$(lumpSumBenefitJEl).on('change', function(){
						var currentVal = $(this).val();
						sessionStorage.setItem('lumpSumBenefitVal', currentVal );
					});
