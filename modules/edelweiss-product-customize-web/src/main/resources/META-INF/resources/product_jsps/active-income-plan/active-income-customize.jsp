...
<c:choose>
	<c:when test="${isProductSelected}">
					...
					<div class="edelweissFormInner d-flex">
						<div id="edelweissTokioForm1" class="edelweissTokioForm">
							<div class="row owl-carousel owl-theme">
								...
								<c:if test="${productCustomizeSummaryData['productDefaultValueData'].isIncomeOptionApplicable()}">
									<div class="item">
										<div class="labelInputGroup">
											<div class="labelGroup">
												<label for="<portlet:namespace />incomeOption" class="pform-title mb-0"><liferay-ui:message key="income-option" /></label>
												<p class="premiuim-small-text">
													<liferay-ui:message key="select-your-income-option" />
												</p>
											</div>
											<div class="select-container">
												<select class="form-control" id="<portlet:namespace />incomeOption" name="<portlet:namespace />incomeOption">
													<c:forEach var="curIncomeOption" items="${incomeOptionsData}">
														<option data-name="${curIncomeOption.key}" value="${curIncomeOption.key}" ${productCustomizeSummaryData['productDefaultValueData'].incomeOption.key == curIncomeOption.key ? 'selected' : ''}>${curIncomeOption.value}</option>
													</c:forEach>
												</select>
											</div>
										</div>
								...
								<c:if test="${productCustomizeSummaryData['productDefaultValueData'].isAIPFamilyBenefitApplicable()}">
									<div class="item">
										<div class="labelInputGroup">
											<div class="labelGroup">
												<label for="<portlet:namespace />familyIncomeBenefit" class="pform-title mb-0"><liferay-ui:message key="family-income-benefit" /></label>
												<p class="premiuim-small-text">
													<liferay-ui:message key="add-family-income-benefit" />
												</p>
											</div>
											<div class="select-container">
												<select class="form-control" id="<portlet:namespace />familyIncomeBenefit" name="<portlet:namespace />familyIncomeBenefit">
													<c:forEach var="curInvestingFor" items="${aipFamilyIncomeBenefitData}">
														<option value="${curInvestingFor.key}" ${productCustomizeSummaryData['productDefaultValueData'].aipFamilyIncomeBenefit.key == curInvestingFor.key ? 'selected' : ''}>${curInvestingFor.value}</option>
													</c:forEach>
												</select>
											</div>
										</div>
							...
							<!-- Maturity Age Starts -->
								<c:if test="${productCustomizeSummaryData['productDefaultValueData'].isMaturityAgeApplicable()}">
									<div class="item">
										<div class="labelInputGroup">
											<div class="labelGroup">
												<label for="<portlet:namespace />maturityAge" class="pform-title mb-0"><liferay-ui:message key="maturity-age" /></label>
												<p class="premiuim-small-text">
													<liferay-ui:message key="maturity-age" />
												</p>
											</div>
											<div class="select-container">
												<select class="form-control" id="<portlet:namespace />maturityAge" name="<portlet:namespace />maturityAge">
													<c:forEach var="curInvestingFor" items="${aipMaturityAgeData}">
														<option value="${curInvestingFor.key}" ${productCustomizeSummaryData['productDefaultValueData'].maturityAge.key == curInvestingFor.key ? 'selected' : ''}>${curInvestingFor.value} <liferay-ui:message key="years" /></option>
													</c:forEach>
												</select>
											</div>
										</div>
							...
							<!-- Maturity Age Ends -->
								...
								<!-- Guaranteed Income Starts -->
								
								<c:if test="${productCustomizeSummaryData['productDefaultValueData'].isGuaranteedIncomeApplicable()}">
									<div class="item">
										<div class="labelInputGroup">
											<div class="labelGroup">
												<label for="<portlet:namespace />guarantedIncome" class="pform-title mb-0"><liferay-ui:message key="guaranted-income" /></label>
												<p class="premiuim-small-text">
													<liferay-ui:message key="guaranted-income" />
												</p>
											</div>
											<div class="select-container">
												<select class="form-control" id="<portlet:namespace />guarantedIncome" name="<portlet:namespace />guarantedIncome">
												
													<c:forEach var="curInvestingFor" items="${aipMaturityPayoutData}">
													
														<option class="guarantedIncome-${curInvestingFor.value}" value="${curInvestingFor.key}" ${productCustomizeSummaryData['productDefaultValueData'].guaranteedIncome.key == curInvestingFor.key ? 'selected' : ''}>${curInvestingFor.value}</option>
													</c:forEach>
												</select>
											</div>
										</div>
								...
								<div class=" item" id="<portlet:namespace />maturityPayoutOptionDiv">
										<div class="labelInputGroup">
											<div class="labelGroup">
												<label for="<portlet:namespace />maturityPayoutOption" class="pform-title mb-0"><liferay-ui:message key="maturityPayoutOption" /></label>
												<p class="premiuim-small-text">
													<liferay-ui:message key="maturity-payout-option" />
												</p>
											</div>
											<div class="select-container ">
					...
					<div class="edelweissTokioTotalReturns">
							...
							<liferay-ui:message key="label-download-brochure" />
									...
									<div class="premium-form-header">
			                     	<div class="title">
										<liferay-ui:message key="label-total-maturity-benefits" />
									</div>
									</div>
									...
									<div class="perc-proceed-group">
										<div class="button-wrap button-ckeck total-return-value-dropdown">
											<div class="totalnav-dropwdown">
												<div class="labelInputGroup pt-0">
													<div class="select-container">
											...
											<aui:button type="button" id="proceedBtn"
											cssClass="btn btn-primary edto-btn-primary float-right" value="label-proceed">
										    </aui:button>
										</div>
		...
		<%@ include file="product_features.jsp"%>
		<%@ include file="product_riders.jsp"%>
	</c:when>
	<c:otherwise>
...
$(document).ready(function () {
	var config = new Object({}),
		 ...
		 config.pgiRelFieldValues = JSON.parse('${aipPayForValues}');
...
<script src="<%=request.getContextPath()%>/js/active-income-plan.js?t=<%=EdelweissProductCustomizeConstants.CURRENT_TIME_STAMP%>"></script>
<script src="<%=request.getContextPath()%>/js/basic_detail.js?t=<%=EdelweissProductCustomizeConstants.CURRENT_TIME_STAMP%>"></script>
...
$("#<portlet:namespace/>proceedBtn").click(function() {
		...
		returnsAt4Per = totalReturnsAmountRef['4']; 
		returnsAt8Per = totalReturnsAmountRef['8']; 
		planOption = $('#<portlet:namespace />incomeOption').find(":selected").text();
		premiumPaymentTerm = $('#<portlet:namespace/>policyFor').find(":selected").text();
		benefitDuration = $('#<portlet:namespace />maturityAge').find(":selected").text();
		...
		benefitType = $('#<portlet:namespace />guarantedIncome').find(":selected").text();
