(function($, edelweissProductCustomizePortlet) {
	...
	function viewProductCustomize(config){
		var namespace = config.namespace,
			...
			policyForRelData = config.policyForRelData,
			policyForRelJSONData =JSON.parse(policyForRelData),
			...
			risingStarModalJEl = config.risingStarModalJEl,
			...
			isRisingStarBenefitOptedJEl = config.isRisingStarBenefitOptedJEl,
			isRisingStarBenefitOpted = config.isRisingStarBenefitOpted,
			...
			addRisingStarBenefitURL = config.addRisingStarBenefitURL,
			...
			removeLittleChampJEl = "#" + namespace + "removeRisingStar",
		...
		var SWPfrequencyMap = {};
			SWPfrequencyMap["Annual"] = 117; 
			SWPfrequencyMap["Semi-Annual"] = 118; 
			SWPfrequencyMap["Quarterly"] = 119;
			SWPfrequencyMap["Monthly"] = 120; 
		 ...
		 * Load Policy Terms
		 */	
		welathPlusInitPolicyTermPolicyForEls = function(){
			for (var curPolicyTerm in policyTermPolicyForRelJSONData) {
		 ...
		 */
		onPolicyTermChange = function(){
			$(policyForJEl).find("option").remove();
			var policyTerm = $(policyTermJEl).val();
			updatePayForDropdown(policyTerm);
			
			welathPlusGenerateBIRequest();
		};
		
		updatePayForDropdown = function(selectedPolicyTerm) {
			for (var curPolicyFor in policyForRelJSONData)  {
				if(Number(selectedPolicyTerm) >= Number(curPolicyFor)){
					var selected = curPolicyFor == selectedPolicyTerm ? " selected" : "";
		...
		getBIRequestFundDetails = function() {
			...
			if($(customizeFundTableJEl).find(".allocationAmt").length > 0) {
				...
				$(customizeFundTableJEl + " .allocationAmt").each(function(d) {
					...
					var fundDetailsKey = $(this).data("fundname").replace(/[\s-]/g, '');
				...
				if(allocatedFundDetails && allocatedFundDetails.length > 0) {
					allocatedFundDetails.forEach(curAllocatedFund => {
						...
						var fundDetailsKey = curAllocatedFund["name"].replace(/[\s-]/g, '');
		...
		getBIRequestBasicDetails = function() {
			...
			var proposerDetails = {
				...
				"optionValue1": "0",
				"PR_CHANNEL": "1",
				"INTEREST_RATES": "0.1298518078465487",
				"IS_STAFF": "1",
				"CashFlow": "Y",
				"COMPANY_STATE": "0",
				"AGENT_ID": "TW9001",
		...
		getBIRequestPolicyOptions = function() {
			var isRisingStarBenefitOpted = $(isRisingStarBenefitOptedJEl).val();
			var littleStarBenefitVal = "4";
			if(isRisingStarBenefitOpted && isRisingStarBenefitOpted == "Yes") {
				littleStarBenefitVal = "3";
			}
			
			var policyOptions = {
				"optionId1": littleStarBenefitVal,
			}
		...
		getBIRequestProduct = function() {
			...
			product = $.extend(true,product,getBIRequestPolicyOptions());
			return product;
		 ...
		 * Generate BI Request
		 */
		welathPlusGenerateBIRequest = async function() {
			//Showing Loader
			showLoader();
			if(cookieLeadId) {
		...
		renderAllReturnsTableAndGraph = function(biResponseData) {
			...
			$.each(biJSONArray, function( index, curBIJson) {
				...
				var boosterPremium = curBIJson["BOOSTER_PREMIUM"] ? " ₹ " + Number.parseInt(Math.round(curBIJson["BOOSTER_PREMIUM"])).toLocaleString("en-IN") : "NA";
		...
		onInvestmentAmountBlur = async function() {
			...
			var hasValidationErrors = await welathPlusGenerateBIRequest();
			if(!hasValidationErrors) {
				saveCustomerInvestmentDataHanlder();
			}
		}
		...
		onInvestingForChange = async function() {
			...
			if(investingFor == "Family") {
				...
				$(assuranceDobJEl).val("");
				$(isRisingStarBenefitOptedJEl).val("");
		...
		processSubmitHanlder = async function(e) {
			...
			var isRisingStarBenefitOptedVal = $(isRisingStarBenefitOptedJEl).val();
			if(isRisingStarBenefitOptedVal != "No" && isRisingStarBenefitOptedVal != "Yes" && $(investingForJEl).val() == "Family") {
				$(risingStarModalJEl).modal("show");
				return;
			}
			saveFundDetails();
		...
		initFamilyModalFormValidation = function() {
			if(isRisingStarBenefitOpted == "false" && $(investingForJEl).val() == "Family") {
				$(risingStarModalJEl).modal("show");
			}
		...
		initFamilyModalData = function() {
			...
			var isRisingStarBenefitOptedVal = $(isRisingStarBenefitOptedJEl).val();
			console.log("isRisingStarBenefitOptedVal : initFamilyModalData " + isRisingStarBenefitOptedVal);
			
			if(isRisingStarBenefitOptedVal == "Yes" && $(investingForJEl).val() == "Family") {
		...
		saveFamilyDetails = async function(showBenefitModal) {
			...
			var isRisingStarBenefitOpted = $(isRisingStarBenefitOptedJEl).val();
			if(isRisingStarBenefitOpted == "true") {
				familyDetailsData[namespace + "isRisingStarBenefitOpted"] = isRisingStarBenefitOpted;
			}
		 ...
		 */
		removeLittleChampBenefitHanlder = function() {
		 ...
		 */
		doNotAddLittleChampBenefitHanlder = async function() {
			$(familyModalJEl).modal("hide");
			$(isRisingStarBenefitOptedJEl).val("No");
			...
			var requestData = {};
			requestData[namespace + "isRisingStarBenefitOpted"] = "No";
			requestData[namespace + "customerFamilyDetailsId"] = $(customerFamilyDetailsIdJEl).val();
			
			$.ajax({
				url:addRisingStarBenefitURL,
				type:"post",
				dataType: "json",
				data : requestData,
				async: true,
			...
			$(risingStarModalJEl).modal("hide");
			var hasValidationErrors = await welathPlusGenerateBIRequest();
			console.log("hasValidationErrors doNotAddLittleChampBenefitHanlder : " + hasValidationErrors);
			if(!hasValidationErrors) {
		...
		updateLMSLead = function() {
			...
			var isRisingStarBenefitOptedValue = $(isRisingStarBenefitOptedJEl).val();
			var RisingStarBenefit_YN = "";
			if(isRisingStarBenefitOptedValue == "Yes") {
				RisingStarBenefit_YN = "Y";
			} else if(isRisingStarBenefitOptedValue == "No") {
				RisingStarBenefit_YN = "N";
			}
			updateLMSLeadData[namespace + "IS_LA_PR_Same_YN"] = "Y";
			if(investingForValue == "Family") {
				updateLMSLeadData[namespace + "IS_LA_PR_Same_YN"] = "N";
			}
			updateLMSLeadData[namespace + "RisingStarBenefit_YN"] = RisingStarBenefit_YN;
		 ...
		 */
		addLittleChampBenefitHanlder = async function() {
			$(isRisingStarBenefitOptedJEl).val("Yes");
			
			$(removeLittleChampJEl).val("false");
			$(removeLittleChampWrapperJEl).show();
			
			var requestData = {};
			requestData[namespace + "isRisingStarBenefitOpted"] = "Yes";
			requestData[namespace + "customerFamilyDetailsId"] = $(customerFamilyDetailsIdJEl).val();
			$.ajax({
				url:addRisingStarBenefitURL,
				type:"post",
			...
			var hasValidationErrors = await welathPlusGenerateBIRequest();
			console.log("hasValidationErrors addLittleChampBenefitHanlder : " + hasValidationErrors);
			if(!hasValidationErrors) {
				var isFormSubmitPending = $(isFormSubmitPendingJEl).val();
				if(isFormSubmitPending && isFormSubmitPending == "true") {
					$(customizeFormJEl).submit();
		...
		initEvents = function(){
			...
			$(addLittleChampBenefitJEl).click(addLittleChampBenefitHanlder);
			$(doNotaddOnJEl).click(doNotAddLittleChampBenefitHanlder);
			$(removeLittleChampBenefitLabelJEl).click(removeLittleChampBenefitHanlder);
		};
		
		welathPlusInitDefaultProductValues = function() {
		...
		init = async function(){
			initEvents();
			welathPlusInitPolicyTermPolicyForEls();
			var hasDefaultProdcutVal = welathPlusInitDefaultProductValues();
