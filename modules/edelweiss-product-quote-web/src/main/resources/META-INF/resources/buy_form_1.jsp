...
<portlet:actionURL name="add/quote_journey" var="actionURL" />
...
<form name="generalForm" id="generalForm" method="POST"
	...
	<div class="row">
		...
		<div class="edto-check-content">
			<div class="form-group">
				<input type="checkbox" id="sutabilityId" name='<portlet:namespace/>suitabiltyMatrix' for="suitabiltyMatrix" 
					<c:if test="${cookieLeadValue['suitabilityConsent'] == 'true' || empty cookieLeadValue['suitabilityConsent']}"> checked </c:if>>
				<label for="sutabilityId"> <liferay-ui:message key="suitability-consent" /></label>
			</div>
			<div class="form-group">
				<input type="checkbox" name='<portlet:namespace/>consenttermsandcondition' 
					readonly="readonly" value="Consent" onclick="return false" checked="checked" for="flexCheckDefault">
				<label for="flexCheckDefault"> <liferay-ui:message key="ndnc-consent" /></label>
			</div>
		</div>
		...
		<div class="edto-bullet1 mb-3 mt-3">
			<div>
				<button type="submit" class="edto-btn-primary" id='prequote-submit'
					...
					<liferay-ui:message key="button-text-buy-${productCode}" />
				</button>
			</div>
		</div>
	</div>
