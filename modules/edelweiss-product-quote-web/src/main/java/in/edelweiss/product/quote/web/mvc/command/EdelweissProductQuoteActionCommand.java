...
import static in.edelweiss.tokio.constants.EdelweissObjectConstants.SOURCE_OF_TRAFFIC;
...
@Component(immediate = true, property = {
		...
		"mvc.command.name=add/quote_journey" }, service = MVCActionCommand.class)
public class EdelweissProductQuoteActionCommand extends BaseMVCActionCommand {
	@Reference
	...
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		log.debug("EdelweissProductQuoteActionCommand >> doProcessAction >> prodcut name :: " + productName + " product id :: " + productCode);
		...
		mapformMap.put(RECOMMENDED_OBJECTIVE, investmentObjectiveMap.get(ParamUtil.getString(actionRequest, INVESTMENT_OBJECTIVE)));
		...
		if (Validator.isNotNull(leadId)) {
			...
			log.debug("EdelweissProductQuoteActionCommand >>>> doProcessAction >>> updateRequesBody  ::: "+leadUpdateRequestBody);
			JSONObject updateLeadResponse = edelweissTokioCommonApi.updateLMSLead(leadUpdateRequestBody);
			log.debug("EdelweissProductQuoteActionCommand >>>> doProcessAction >>>> Lead Update Successully ::: "+updateLeadResponse);
			
			mapformMap.put(LEAD_STATUS, updateLeadResponse.getJSONObject(RESPONSE_DATA).getString(LEAD_STATUS));
			mapformMap.put(LMS_INTEREST_ID, updateLeadResponse.getJSONObject(RESPONSE_DATA).getString(LMS_QUOTE_INTEREST_ID));
			mapformMap.put(LMS_QUOTE_ID, updateLeadResponse.getJSONObject(RESPONSE_DATA).getString(LMS_QUOTE_JOURNEY_ID));
			...
			JSONObject generatedLeadId = edelweissTokioCommonApi.generateLead(mapformMap, productName, apiResponseObject, actionRequest.getUserAgent());
			log.debug("EdelweissProductQuoteActionCommand >>>> doProcessAction >>> Lead Id Generated  :: "+ generatedLeadId);
			...
			if(Validator.isNotNull(generatedLeadId) && generatedLeadId.getBoolean(STATUS)) {
				...
				mapformMap.put(LMS_INTEREST_ID, generatedLeadId.getJSONObject(RESPONSE_DATA).getString(LMS_QUOTE_INTEREST_ID));
				...
				log.debug("EdelweissProductQuoteActionCommand >>> doProcessAction >>> lr product response  ::: " + lrProductResponse);
			...
			log.error("Unable to Submit buy form 1: "+e);
			actionRequest.setAttribute("showErrorModal", true);
		}
	}
