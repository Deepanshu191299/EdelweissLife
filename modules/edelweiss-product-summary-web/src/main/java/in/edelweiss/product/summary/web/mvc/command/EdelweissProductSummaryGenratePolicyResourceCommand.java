...
@Component(
		...
		"mvc.command.name=/save/GeneratePolicy" + "" }, 
		service = MVCResourceCommand.class)
public class EdelweissProductSummaryGenratePolicyResourceCommand extends BaseMVCResourceCommand {
	...
	private static final Log log = LogFactoryUtil.getLog(EdelweissProductSummaryGenratePolicyResourceCommand.class);

	@Override
	protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
		log.info("EdelweissProductSummaryGenratePolicyResourceCommand >>> Called :::" );
		JSONObject response = JSONFactoryUtil.createJSONObject();
		boolean cutomerInvestmentStatus = false;
		String message = StringPool.BLANK;
		String pfRedirectURL = StringPool.BLANK;
		...
		try { 
			...
			String leadId = edelweissTokioCommonAPI.getLeadIdFromCookie(resourceRequest, EdelweissObjectConstants.LEAD_ID);
			...
			if(Validator.isNull(policyResponse.getJourneyURL())) {
				...
				cutomerInvestmentStatus = true;
				pfRedirectURL = redirectURL;
				response.put("status", cutomerInvestmentStatus);
		        response.put("message", message);
		        response.put("pfRedirectURL", pfRedirectURL);
			...
			log.info("customerPolicyJsonReq for "+applicationNumber+" "+customerPolicyJsonReq);
			...
			log.info("CustomerPolicyJsonRes for "+applicationNumber+" "+customerPolicyJsonRes);
			// Check if 'applicationNumber' exists and is not empty
			if (customerPolicyJsonRes.has("applicationNumber")) {
			    String applicationId = customerPolicyJsonRes.getString("applicationNumber");
				log.info("ApplicationNumber found in the customerPolicyJsonRes ::: "+applicationId);
			} else {
				log.info("ApplicationNumber not found in the customerPolicyJsonRes ::: "+applicationNumber);
				cutomerInvestmentStatus = false;
			...
			cutomerInvestmentStatus = true;
			pfRedirectURL = policyResponse.getJourneyURL();
			response.put("status", cutomerInvestmentStatus);
	        response.put("message", message);
	        response.put("pfRedirectURL", pfRedirectURL);
			resourceResponse.setContentType("application/json");
			...
			cutomerInvestmentStatus = false;
			message = "An error occurred getting the customer details";
			log.error("EdelweissProductSummaryGenratePolicyResourceCommand >>> serveResource >>> An error occurred getting the customer details ::: " + e);
		}catch (JsonMappingException e) {
			cutomerInvestmentStatus = false;
			message = "An error occurred while mapping the response to the object";
			log.error("EdelweissProductSummaryGenratePolicyResourceCommand >>> serveResource >>> An error occurred while mapping the response to the object ::: " + e);
		}catch (JsonProcessingException e) {
			cutomerInvestmentStatus = false;
			message = "An error occurred while processing the reponse to the object";
			log.error("EdelweissProductSummaryGenratePolicyResourceCommand >>> serveResource >>> An error occurred while processing the reponse to the object ::: " + e);
		}catch (UnsupportedEncodingException e) {
			cutomerInvestmentStatus = false;
			message = "An error occurred while encoding the URL";
			log.error("EdelweissProductSummaryGenratePolicyResourceCommand >>> serveResource >>> An error occurred while encoding the URL ::: " + e);
		}catch (Exception e) {
			cutomerInvestmentStatus = false;
			message = "Internal Server Error";
			log.error("EdelweissProductSummaryGenratePolicyResourceCommand >>> serveResource >>> Internal Server Error ::: " + e);
		}
