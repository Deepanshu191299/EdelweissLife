				...
				<div class="panel-group panel-group-flush">
					...
					<aui:fieldset collapsed="<%= true %>" collapsible="<%= true %>" label="configuration">
						...
						<c:if test="<%= PropsValues.BLOGS_TRACKBACK_ENABLED %>">
                         ...
                         <!-- New Field to add initial view count for a Blogs during update processes -->
                         <c:if test="<%= entry != null %>">
                        
                        <% AssetEntry assetEntry = AssetEntryLocalServiceUtil.getEntry(BlogsEntry.class.getName(), blogsEditEntryDisplayContext.getEntryId());
                           long viewCount = ViewCountManagerUtil.getViewCount(themeDisplay.getCompanyId(), ClassNameLocalServiceUtil.getClassNameId(AssetEntry.class), assetEntry.getEntryId());
                        %>
						        <aui:input helpMessage="Manually define No. of Views to be displayed on the website. Values GREATER than current views are Valid." label="Current Views" type="text" name="initialViewsCount" value="<%= viewCount%>" >
						        	<aui:validator name="min"><%=viewCount %></aui:validator>
						        </aui:input>
						</c:if>
					</aui:fieldset>
...
$(document).ready(function() {
	$("#<portlet:namespace/>related-assetsContent").on('click', function(){
		var divContainerId = $("#<portlet:namespace/>related-assetsContent").find("button").attr("aria-controls");
		$("#"+divContainerId).find("button").filter(function(){
		    if(!$(this).attr("data-href").includes("<%= BlogsEntry.class.getName() %>")){
		        $(this).remove();
		    }
		});
	});
