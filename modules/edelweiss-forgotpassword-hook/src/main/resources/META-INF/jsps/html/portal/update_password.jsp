...
<div class="login-card-container eli-custom-login">
	<div class="card login-card">
		<div class="card-body">
			<h4 class="card-title text-center"><liferay-ui:message key="<%= titlePage %>" /></h4>
			...
			<c:choose>
				...
				<c:otherwise>
					<aui:form action='<%= themeDisplay.getPathMain() + "/portal/update_password" %>' method="post" name="fm">
						...
						<aui:button-row>
							<aui:button cssClass="edto-btn-primary btn-secondary" type="submit" />
	
							<c:if test="<%= showCancelButton %>">
								<aui:button cssClass="edto-btn-secondary" href='<%= themeDisplay.getPathMain() + "/portal/logout" %>' type="cancel" />
							</c:if>
						</aui:button-row>
					</aui:form>
				</c:otherwise>
...
.eli-custom-login .edto-btn-secondary{
	background: #0c57a5 !important;
    border: 0.5px solid #8197af !important;
