...
<div id="proposer_family_history_details_fields_container" class="medical-test">
	<div class="radio-wrapper" id="pr_sp_family_history_details_fields_wrapper">
		<div class="location-field">
					...
					<div class="input-box">
						<h4>
							<c:if test="${isProposerApplicable}">
								<liferay-ui:message key="label-family-details-proposer" />
							</c:if>
							
							<c:if test="${isSpouseApplicable}">
								<liferay-ui:message key="label-family-details-spouse" />
							</c:if>
						</h4>
					</div>
				</div>
			...
			<div class="row">
				<div class="col-md-3 col-12">
					<div class="form-box">
						<div class="select-container">
							<aui:select title=" " name="pr_sp_fh_family_member_data_id" label="" cssClass="customValidate select2-select relationDropdown"
				...
				<div class="col-md-3 col-12">
					<div class="form-box">
						<div class="select-container">
							<aui:select title=" " name="pr_sp_fh_health_status" label="" cssClass="customValidate select2-select"
				...
				<div class="col-md-3 col-12 prSPFhFieldsForNonDeceased">
					<div class="form-box">
						<label class="custom-field two"> 
							<aui:input name="pr_sp_fh_age"
							onkeyup="validateRadioButtonsOnChange('other_details_form','pr_sp_fh_age')"
								placeholder="&nbsp;" label="" cssClass="customValidate" type="text"
								oninput="this.value = this.value.replace(/[^0-9]/g, '');" maxLength="3" minLength="0"
				...
				<div class="col-md-3 col-12 prSPFhFieldsForDeceased">
					<div class="form-box">
						<label class="custom-field two"> 
							<aui:input name="pr_sp_fh_age_on_death"
							onkeyup="validateRadioButtonsOnChange('other_details_form','pr_sp_fh_age_on_death')"
								placeholder="&nbsp;" label="" cssClass="customValidate" type="text"
				...
				<div class="col-md-3 col-12 prSPFhFieldsForDeceased">
					<div class="form-box">
						<div class="select-container">
							<aui:select title=" " name="pr_sp_fh_cause_of_death" label="" cssClass="customValidate select2-select"
			...
			<div class="row">
				<div class="col-md-4 col-12">
					<aui:button value="label-add-member" cssClass="mt-2 edto-btn-primary addNewRecord" id="addProposerFamilyHistoryDetails" 
						data-table-id = "pr_sp_family_history_details_table" 
						data-fieldset-id="pr_sp_family_history_details_fields_wrapper"/>
				</div>
			</div>
			<p class="p-wrap-class"><liferay-ui:message key="label-click-on-add-member-to-save-details" /></p>
		</div>
	</div>
	<div id="error-messagePrSp" style="display: none; color: red;">Please add parents and spouse(if married) details.</div>
	<!-- Table Here -->
	<div class="other-details-table-section" id="pr_sp_family_history_details_table_wrapper">
		<div class="table-title-wrapper mt-4">
			<h5 class="pt-0">Member Details</h5>
			<div class="tbl-srol-btn">
			<button id="slideBack-lasp1" type="button" disabled=""> <span><svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<button id="slide-lasp1" type="button"> <span><svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		<div class="edto-custom-table pb-0" id="other_details_table5">

				<table id="pr_sp_family_history_details_table" class="table table-bordered w-100 other_details_table"
					data-param="pr_sp_family_history_list" data-index-param="fh_row_id" data-fieldset-id="pr_sp_family_history_details_fields_wrapper">
					<thead>
						<tr>
							<th class="userdata" data-input-name="<portlet:namespace/>pr_sp_fh_family_member_data_id"
								data-param="family_member_data_id" data-master="Family History Relation">
								Relation
							</th>
							<th class="userdata" data-input-name="<portlet:namespace/>pr_sp_fh_age"
								data-param="age">
								Living Age
							</th>
							<th class="userdata" data-input-name="<portlet:namespace/>pr_sp_fh_age_on_death"
								data-param="age_on_death">
								Age at Death
							</th>
							<th class="userdata" data-input-name="<portlet:namespace/>pr_sp_fh_health_status"
								data-param="health_status" data-master="Health Status">
								Health Status
							</th>
							<th class="userdata" data-input-name="<portlet:namespace/>pr_sp_fh_cause_of_death"
	...
	<div class="radio_container">
		<aui:input name='pr_sp_fh_any_disease_yn' id="pr_sp_fh_any_disease_yes" cssClass="validate hasDependentFields prsp_radioYes requiredRadio"
			type="radio" label="" required="required" value="Y"
			checked="${prSpFamilyHistoryDetails.getFhAnyDiseaseYn() == 'Y'}"
			onClick="showDiv(true, 'pr_sp_fh_any_disease_details_field_wrapper')">
		</aui:input>
		<label for="<portlet:namespace/>pr_sp_fh_any_disease_yes">
			<liferay-ui:message key="label-yes" />
		</label>
		<aui:input name='pr_sp_fh_any_disease_yn' id="pr_sp_fh_any_disease_no" cssClass="validate hasDependentFields prsp_radioNo requiredRadio" 
			type="radio" label="" required="required" value="N"
			checked="${prSpFamilyHistoryDetails.getFhAnyDiseaseYn() == 'N'}"
			onClick="showDiv(false, 'pr_sp_fh_any_disease_details_field_wrapper')">
		</aui:input>
		<label for="<portlet:namespace/>pr_sp_fh_any_disease_no">
			<liferay-ui:message key="label-no" />
		</label>
	</div>
	<label class="custom-field two has-error requiredRadioMessage" id="<portlet:namespace/>pr_sp_fh_any_disease_yn_required"
		style="display: none;">
	...
	<div class="form-box" id="pr_sp_fh_any_disease_details_field_wrapper" style="display:none;">
		<label class="custom-field two"> 
			<aui:input name="pr_sp_fh_any_disease_details"
			oninput="validateRadioButtonsOnChange('other_details_form','pr_sp_fh_any_disease_details')"
				placeholder="&nbsp;" label="" cssClass="validate valphanumwithspace" type="text"
				value="${prSpFamilyHistoryDetails.getFhAnyDiseaseDetails()}" maxLength="75" minLength="0" wrappedField="<%=true %>">
	...
	<div class="radio_container">
		<aui:input name='pr_sp_fh_etli_policies_yn' id="pr_sp_fh_etli_policies_yes" cssClass="validate prsp_radioYes requiredRadio"
			checked="${prSpFamilyHistoryDetails.getFhEtliPoliciesYn() == 'Y'}"
			type="radio" label="" required="required" value="Y">
		</aui:input>
		<label for="<portlet:namespace/>pr_sp_fh_etli_policies_yes">
			<liferay-ui:message key="label-yes" />
		</label>
		<aui:input name='pr_sp_fh_etli_policies_yn' id="pr_sp_fh_etli_policies_no" cssClass="validate prsp_radioNo requiredRadio"
			checked="${prSpFamilyHistoryDetails.getFhEtliPoliciesYn() == 'N'}"
			type="radio" label="" required="required" value="N">
		</aui:input>
		<label for="<portlet:namespace/>pr_sp_fh_etli_policies_no">
			<liferay-ui:message key="label-no" />
		</label>
	</div>
	<label class="custom-field two has-error requiredRadioMessage" id="<portlet:namespace/>pr_sp_fh_etli_policies_yn_required"
		style="display: none;">
		<div class="form-feedback-item form-validator-stack help-block">
			<div role="alert" class="required">
				<liferay-ui:message key="this-field-is-required" />
...
$(document).ready(function(){
	var container = document.getElementById('other_details_table5');
	var button = document.getElementById('slide-lasp1');
	var back = document.getElementById('slideBack-lasp1');
